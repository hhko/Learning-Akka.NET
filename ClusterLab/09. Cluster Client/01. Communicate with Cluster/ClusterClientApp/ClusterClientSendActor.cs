...
namespace ClusterClientApp
{
    public class ClusterClientSendActor : ReceiveActor
    {
        ...
        public ClusterClientSendActor()
        {
            ...
            c.Tell(new ClusterClient.Send("/user/FooActor1", "hello", localAffinity: true));
            c.Tell(new ClusterClient.SendToAll("/user/FooActor2", "hi"));

            //
            // 사용자 정의 메시지는 Seed Node에 참조되어야 한다.
            // TODO: 해결 방법이 없을까?, 메시지를 받는 Node만 참조할 수 없을까?
            //
            c.Tell(new ClusterClient.Send("/user/FooActor1", new Welcome()));
        }
    }
}
